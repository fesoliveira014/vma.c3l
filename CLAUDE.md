...
- `linked-libs/<target>/` — per-target compiled VMA libs. **Not tracked in git** (`linked-libs/*/` is ignored). `linux-x64` and `windows-x64` are built by `.github/workflows/build-vma-libs.yml`, which on a `v*` tag publishes them as GitHub release assets together with a `vma.c3l-<tag>.zip` bundle; `ci.yml` builds `linux-x64` before linking the smoke harness. Locally: `scripts/fetch-vma-headers.sh <dir>` then `VULKAN_SDK=<dir> scripts/build-vma.sh`. The other targets are not yet populated.
- `docs/`, `.claude/`, `.superpowers/` — local-only agent/dev-process files, gitignored. The `@docs/...` imports below resolve only in a checkout that has them.
